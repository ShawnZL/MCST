...
class State(object): # 一切都是object
    ...
    def get_cumulative_choices(self):
        if len(self.cumulative_choices):
    ...
    # 根据决定修改choice
    def get_next_state_with_choice(self, choice):
        next_state = State()
        next_state.set_current_value(self.current_value + choice)
...
def expand(node):
    ...
    if len(tried_sub_node_states) == 4:  # 表示所有节点都已经遍历到，则返回不能增加
        pass
    else:
        # 新增加的内容，方式进入while循环无法结束
        tried_sub_node_actions = [
            sub_node_states.get_cumulative_choices for sub_node_states in tried_sub_node_states
        ]
...
def best_child(node, is_exploration):
    ...
    :param node:
    :param is_exploration: 是否是exploration阶段
    """
